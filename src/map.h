...
/** @brief Sprawdza, czy dane miasto nie zawiera znaków zabronionych.
 * @param[in] city - wskaźnik na napis reprezentujący nazwę miasta;
 * @return Wartość @p true, jeśli nazwa miasta jest poprawna.
 * Wartość @p false, jeśli nazwa miasta jest niepoprawna.
 */
...
/** @brief Jeżeli newSpecificRoute zakończyło się pomyślnie, to ta funkcja
 * uzupełnia mapę o odcinki drogi z danej drogi krajowej.
 * @param[in] map - wskaźnik na mapę dróg;
 * @param[in] city - wskaźnik na napis reprezentujący ostatnie miasto z danej drogi krajowej;
 */
void supplementRoads(Map* map, const char* city);

/**@brief Dodaje odcinek drogowy city1 - city2 o danej długości i roku do danej drogi krajowej.
 *Jeśli wystąpi błąd droga krajowa jest usuwana.
 *@param[in] map - wskaźnik na mapę dróg;
 *@param[in] city1 - wskaźnik na napis reprezentujący nazwę miasta;
 *@param[in] city2 - wskaźnik na napis reprezentujący nazwę miasta;
 ...
 *@param[in] routeId - numer drogi krajowej;
 *@param[in] tail - wskaźnik na ostatni element listy odpowiadającej danej drodze krajowej;
 *@return Wskaźnik na ogon listy odpowiadającej danej drodze krajowej lub NULL, jeżeli wystąpił błąd, tj.
 *wystąpiła pętla, podana długość nie zgadza się z długością danej drogi zapisaną w mapie lub
 *podany rok jest mniejszy niż rok danej drogi zapisany w mapie.
 */
...
/** @brief Usuwa drogę krajową o danym numerze.
 * @param[in] map - wskaźnik na mapę dróg;
 * @param[in] routeId - numer drogi krajowej;
 */
